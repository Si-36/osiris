...
class KnowledgeGraphManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            print(
                f"✅ Knowledge graph initialized - Database: {self.config.neo4j_database}"
            )
    ...
    async def record_workflow_event(
        self, context: ObservabilityContext, state: Dict[str, Any]
    ) -> None:
        """
        ...
        workflow_event = {
            ...
            "system_health_score": state.get("system_health", {}).get(
                "health_score", 0.0
            ),
    ...
    async def _process_workflow_batch(
        self, session, workflows: List[Dict[str, Any]]
    ) -> None:
    ...
    async def _process_evidence_batch(
        self, session, evidence: List[Dict[str, Any]]
    ) -> None:
    ...
    async def _process_decision_batch(
        self, session, decisions: List[Dict[str, Any]]
    ) -> None:
    ...
    async def get_historical_context(
        self, current_evidence: List[dict], top_k: int = 3
    ) -> List[dict]:
        """
        ...
        current_evidence_types = [
            e.get("evidence_type", "unknown") for e in current_evidence
        ]
