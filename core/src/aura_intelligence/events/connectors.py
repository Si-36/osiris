...
connector_errors = meter.create_counter(
    name="connector.errors", description="Number of connector errors", unit="1"
)
...
class TemporalKafkaConnector(EventProcessor):
        ...
        with tracer.start_as_current_span(
            ...
            try:
                ...
                connector_errors.add(
                    1, {"connector": "temporal", "error": type(e).__name__}
                )
    ...
    async def _monitor_workflow_result(self, event_id: str, handle: Any) -> None:
        ...
        try:
            ...
            # Publish to Kafka
            await self.producer.send_event("workflow.completed", completion_event)
            ...
            await self.producer.send_event("workflow.failed", failure_event)

    async def _publish_error_event(
        self, original_event: EventSchema, error: Exception
    ) -> None:
...
class StateStoreConnector:
    ...
    async def store_workflow_state(self, event: WorkflowEvent) -> None:
        ...
        if event.event_type == EventType.WORKFLOW_STATE_CHANGED:
            ...
            await self.redis.setex(
                state_key, int(self.retention_period.total_seconds()), json.dumps(state)
            )
...
class CDCConnector:
    ...
    def __init__(
        self, producer: TransactionalProducer, database_config: Dict[str, Any]
    ):
        self.producer = producer
...
def create_temporal_connector(
    temporal_host: str = "localhost:7233", kafka_config: Optional[ProducerConfig] = None
...
def create_state_connector(
    redis_url: str = "redis://localhost:6379", postgres_url: Optional[str] = None
) -> StateStoreConnector:
...
def create_cdc_connector(
    database_config: Dict[str, Any], kafka_config: Optional[ProducerConfig] = None
) -> CDCConnector:
