...
stream_state_size = meter.create_gauge(
    name="stream.state.size", description="Size of stream state"
)

stream_window_latency = meter.create_histogram(
    name="stream.window.latency", description="Window processing latency", unit="ms"
)
...
class AgentEventStream(EventProcessor):
    ...
    def _build_topology(self) -> StreamTopology:
        ...
        # Define topology
        topology.add_source("agent_events", ["agent.events"]).add_processor(
            "performance_tracker",
        ...
        if self.config.output_topic:
            topology.add_sink("output", self.config.output_topic, ["anomaly_detector"])
    ...
    async def process(self, event: EventSchema) -> None:
        ...
        with tracer.start_as_current_span(
            ...
            try:
                ...
                if anomaly and self.producer and self.config.output_topic:
                    await self.producer.send_event(self.config.output_topic, anomaly)
    ...
    async def _process_window(self, window_key: str, events: List[AgentEvent]) -> None:
        """Process a completed window of events."""
        start_time = datetime.now(timezone.utc)

        with tracer.start_as_current_span(
            "stream.window.process",
            attributes={"window.key": window_key, "window.size": len(events)},
        ) as span:
            try:
                ...
                logger.info(
                    "Window processed", window_key=window_key, aggregation=aggregation
                )
        ...
        if event.event_type == EventType.WORKFLOW_COMPLETED:
            ...
            if state and state.get("total_duration_ms", 0) > 300000:  # 5 minutes
                ...
                await self.sla_violations.put(
                    f"{event.workflow_id}:{event.timestamp.timestamp()}", violation
                )
