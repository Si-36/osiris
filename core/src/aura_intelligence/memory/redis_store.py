...
class RedisConfig:
    ...
    socket_keepalive: bool = False  # Disable keepalive to avoid connection issues
    socket_keepalive_options: Dict[int, int] = None
    metrics_update_interval: int = 30  # seconds
    
    def __post_init__(self):
        if self.socket_keepalive_options is None:
            # Disable socket keepalive options to avoid connection issues
...
# -        )
        # Simple Redis connection to avoid connection issues
        try:
            self.redis = redis.Redis(
                host=self.config.url.split("://")[1].split(":")[0],
            ...
            # Fall back to connection pool approach
            self.pool = redis.BlockingConnectionPool.from_url(
                self.config.url,
                max_connections=self.config.max_connections,
                socket_timeout=self.config.socket_timeout,
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check if we have connection pool (fallback mode doesn't have it)
            pool_info = {}
