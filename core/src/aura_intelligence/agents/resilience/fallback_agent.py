...
from ..observability import GenAIAttributes
from aura_intelligence.agents.observability import AgentInstrumentor
...
class FallbackAgent(
    AgentBase[TInput, TOutput, TState], Generic[TInput, TOutput, TState]
):
    """
    ...
    def __init__(
        ...
        self.logger = structlog.get_logger().bind(
            agent=config.name, strategy=fallback_strategy.value
        )
    ...
    def add_alternative_agent(self, agent: AgentBase[TInput, TOutput, TState]) -> None:
        ...
        self.logger.info("Added alternative agent", alternative_agent=agent.name)
    ...
    async def _execute_step(self, state: TState, step_name: str) -> TState:
        ...
        try:
            ...
            return await self.circuit_breaker.call(
                self.primary_agent._execute_step, state, step_name
            )
    ...
    async def _process(self, input_data: TInput) -> TOutput:
        ...
        with tracer.start_as_current_span(
            ...
            try:
                ...
                result = await self.circuit_breaker.call(
                    self.primary_agent._process, input_data
                )
        ...
        async def _execute_fallback_strategy(
            self, input_data: TInput, error: Exception
        ) -> TOutput:
            pass
        ...
        try:
            ...
            self.logger.error("Simplified model also failed", error=str(e))
            return await self._fallback_partial_response(input_data)
        ...
        for agent in self.alternative_agents:
            try:
                self.logger.info("Trying alternative agent", agent=agent.name)
                return await agent._process(input_data)
            except Exception as e:
                self.logger.warning(
                    "Alternative agent failed", agent=agent.name, error=str(e)
                )
                continue
