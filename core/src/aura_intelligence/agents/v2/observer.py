...
from aura_intelligence.observability.knowledge_graph import (
...
class ObserverAgentV2(AgentBase[Dict[str, Any], Dict[str, Any], AgentState]):
    ...
    async def observe_system_step(self, state: AgentState) -> AgentState:
        ...
        metrics, processes, events = await asyncio.gather(
            observe_metrics(), observe_processes(), observe_events()
        )
        ...
        if metrics["cpu_usage"] > 80:
            observations["anomalies"].append(
                {"type": "high_cpu", "value": metrics["cpu_usage"], "threshold": 80}
            )
    ...
    async def enrich_observations_step(self, state: AgentState) -> AgentState:
        ...
        current_metrics = state.context.get("observations", {}).get(
            "system_metrics", {}
        )
    ...
    def _extract_output(self, final_state: AgentState) -> Dict[str, Any]:
        ...
        return final_state.context.get(
            "observation_report",
            {"error": "No observation report generated", "state": final_state.dict()},
        )
    ...
    async def process_state(self, state: AgentState) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {
                ...
                "duration_ms": (
                    result_state.updated_at - result_state.created_at
                ).total_seconds()
                * 1000,
            },
        }
