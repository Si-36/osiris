...
worker_status = meter.create_gauge(
    name="temporal.worker.status", description="Worker status (1=running, 0=stopped)"
)
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            self.client = await Client.connect(
                self.config.temporal_host, namespace=self.config.namespace
            )
            ...
            self.worker = Worker(
                ...
                graceful_shutdown_timeout=timedelta(
                    seconds=self.config.graceful_shutdown_timeout
                ),
...
class WorkerPool:
    ...
    async def add_worker(self, name: str, task_queue: str, **config_kwargs) -> None:
...
async def create_default_worker_pool() -> WorkerPool:
...
if __name__ == "__main__":
    ...
    async def main():
        worker = await create_worker(task_queue="agent-workflows", identity="worker-1")
