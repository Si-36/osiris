...
class ResearchAgent(AgentBase[str, Dict[str, Any], ResearchAnalysisState]):
    ...
    def build_graph(self) -> StateGraph:
        ...
        async def _execute_step(
            self, state: ResearchAnalysisState, step_name: str
        ...
        async def research_step(
            self, state: ResearchAnalysisState
        ) -> ResearchAnalysisState:
            pass
        ...
        async with self.instrumentor.trace_llm_call(
            ...
            state.decision_reason = (
                f"Found {len(state.research_results)} relevant sources"
            )
        ...
        high_quality_results = [
            r for r in state.research_results if r.get("relevance", 0) > 0.8
        ]

        if len(high_quality_results) >= 2:
            state.last_decision = "validation_passed"
            state.decision_reason = (
                f"{len(high_quality_results)} high-quality sources found"
            )
            state.add_message(
                "assistant",
...
class AnalysisAgent(
    AgentBase[ResearchAnalysisState, Dict[str, Any], ResearchAnalysisState]
):
    """
    ...
    def build_graph(self) -> StateGraph:
        ...
        state.decision_reason = (
            f"Generated {len(state.recommendations)} actionable recommendations"
        )
    ...
    def _create_initial_state(
        self, input_data: ResearchAnalysisState
    ) -> ResearchAnalysisState:
...
class ResearchAnalysisWorkflow:
    ...
    def __init__(self):
        ...
        # Phase 2: Analysis
        analysis_result, analysis_metrics = await self.analysis_agent.process(
            research_state
        )
